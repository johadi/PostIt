...
describe('resetPasswordAction', () => {
  ...
  it('should dispatch RESET_SUCCESSFUL action when user is logged in ',
    (done) => {
      const expectedActions = [{
      ...
      // arguments for reply are (status, data, headers)
      mock.onPost('/\/^api/v1/user/reset-password/', passwordDetail)
        .reply(200, 'password changed successfully');
      mock.onGet('/\/^api/v1/user/reset-password/', passwordDetail)
        .passThrough();
      const store = mockStore({});
      store.dispatch(resetPasswordAction('lord is good' ,passwordDetail)).then(() => {
        expect(store.getActions()).toEqual(expectedActions);
      });
      setTimeout(() => {
        done();
  ...
  it('should dispatch RESET_VALIDATION_ERROR action when ' +
    ...
    (done) => {
      ...
      // arguments for reply are (status, data, headers)
      mock.onPost('/api/v1/user/reset-password?token=xyz', passwordDetail).reply(400, {
  ...
  it('should dispatch RESET_UNSUCCESSFUL action when email is not found',
    (done) => {
      const expectedActions = [{
        type: actionTypes.RESET_VALIDATION_ERROR,
        payload: undefined
